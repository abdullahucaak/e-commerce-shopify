...
test('builds a fixed password recovery path on the current application origin', () => {
  assert.equal(
    passwordRecoveryRedirectUrl('http://127.0.0.1:5174/login?redirect=/dashboard'),
    'http://127.0.0.1:5174/update-password'
  )
  assert.equal(
    passwordRecoveryRedirectUrl('https://manage.yourprostore.ai/anything'),
    'https://manage.yourprostore.ai/update-password'
  )
})
...
test('sends a trimmed email only to the fixed recovery redirect', async () => {
  ...
  }, '  owner@example.com  ', 'http://127.0.0.1:5174/login?redirect=/dashboard')
  assert.deepEqual(received, {
    email: 'owner@example.com',
    options: { redirectTo: 'http://127.0.0.1:5174/update-password' }
  })
})
