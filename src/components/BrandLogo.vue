<template>
  <span
    ...
    :style="{ '--brand-logo-position': position }"
  >
    <img
      v-if="!hasError"
...
const props = defineProps({
  src: {
    type: String,
    default: brand.logo.src
  },
  alt: {
    type: String,
    default: brand.logo.alt
  },
  position: {
    type: String,
    default: brand.logo.position
  },
  trimWhitespace: {
    type: Boolean,
    default: true
...
const normalizeLogo = image => {
  ...
  if (!context) {
    return { src: props.src, ratio: naturalWidth / naturalHeight }
  }
...
const handleLogoLoad = async event => {
  ...
  if (!logoProcessingCache.has(props.src)) {
  ...
  const normalizedLogo = await logoProcessingCache.get(props.src)
  contentRatio.value = normalizedLogo.ratio

  if (normalizedLogo.src !== props.src) {
